...
export default function CampaignsDetailPage() {
  ...
  const filtered = useMemo(() => {
    ...
    rows.sort((a, b) => {
      switch (sort) {
        ...
        case "repeat":
          return b.repeatPct - a.repeatPct;
        case "perScan":
          return b.perScan - a.perScan;
      }
    });
    return rows;
  ...
  const avgPerScan =
    filtered.length === 0
      ? 0
      : filtered.reduce((s, c) => s + c.perScan, 0) / filtered.length;
      ...
      <div style={{ paddingTop: 32 }}>
        ...
        <header className="an2-detail__head">
          <div>
            <p className="an2-eyebrow">
              By campaign · {filtered.length} of {CAMPAIGNS.length}
            </p>
            <h1 className="an2-detail__title">Campaigns</h1>
            <p className="an2-detail__sub">
              Sort by what matters: <strong>Earnings</strong> tells you what
              paid this month. <strong>Repeat %</strong> tells you which
              merchants build long-term value.
              <strong> $/scan</strong> tells you which neighborhood / category
              to lean into.
            </p>
          </div>
          <div
          ...
          >
            ...
            >
              {fmtNum(totalVerified)} verified · avg{" "}
              {fmtUsd(avgPerScan, { cents: true })}/scan
            </p>
          </div>
        </header>
        ...
        >
          <div className="an2-chips">
            {(["all", "active", "ended", "pending"] as const).map((s) => (
              <button
                key={s}
                onClick={() => setStatus(s)}
                className={"an2-chip" + (status === s ? " is-active" : "")}
              >
                {s === "all" ? "All status" : s}
              </button>
            ))}
          ...
          <div className="an2-chips">
            {(["all", "Food", "Beauty", "Lifestyle"] as const).map((c) => (
              <button
                ...
                className={"an2-chip" + (category === c ? " is-active" : "")}
              >
                {c === "all" ? "All cat" : c}
              </button>
            ))}
          ...
          <div className="an2-chips" style={{ marginLeft: "auto" }}>
            {(["earnings", "verified", "repeat", "perScan"] as const).map(
              (s) => (
                <button
                  key={s}
                  onClick={() => setSort(s)}
                  className={"an2-chip" + (sort === s ? " is-active" : "")}
                >
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          {filtered.map((c) => (
            <article
              ...
              style={{
                display: "grid",
                gridTemplateColumns: "minmax(0, 1.6fr) repeat(4, 1fr) auto",
                gap: 24,
                alignItems: "center",
              }}
            >
        ...
        <p className="an2-div">
          <span>{filtered.length} campaigns shown · fin.</span>
        </p>
      </div>
    </div>
  );
