...
import { SUMMARY, TIER_LADDER, tierMeta, nextTier } from "../_data/mock";
...
export default function TierDetailPage() {
  ...
  const tierPct = Math.round(
    ((SUMMARY.currentScore - current.scoreFloor) /
      (next.scoreFloor - current.scoreFloor)) *
      100,
  );
  const currentIdx = TIER_LADDER.findIndex(
    (t) => t.tier === SUMMARY.currentTier,
  );
        ...
        <header className="an2-detail__head">
          <div>
            ...
            <p className="an2-detail__sub">
              Your tier is the lever for everything else — base pay, commission,
              milestone size, concurrent slots, payout speed, merchant access.
              Scores update weekly from completion + reliability + quality +
        ...
        <div className="an2-grid-2">
          ...
          <section className="an2-card">
                ...
                return (
                  <div key={d.name} className="an2-dim">
                    ...
                    <div className="an2-dim__track">
                      <div
                        className={"an2-dim__fill" + (below ? " below" : "")}
                        style={{ width: `${pct}%` }}
                      />
                    </div>
            ...
            >
              ...
              (target 75). One Premium-difficulty campaign with strong merchant
              rating moves Quality +6 → score +1.5 → 3 weeks faster to{" "}
              {next.tier}.
            </div>
          </section>
              ...
              {[
                {
                  ...
                  to: next.commission,
                  note: "per attributed transaction, 30-day window",
                },
                {
                  lbl: "Base pay",
                  from: current.basePay,
                  to: next.basePay,
                  note: "per campaign, before commission",
                },
                {
                  lbl: "Milestone bonus",
                  from: current.milestone
                    ? `+$${current.milestone.bonus} @ ${current.milestone.threshold} txns`
                    : "—",
                  to: `+$${next.milestone!.bonus} @ ${next.milestone!.threshold} txns`,
                  note: "monthly window",
                },
                {
                  lbl: "Concurrent slots",
                  from: `${current.concurrent}`,
                  to: `${next.concurrent}`,
                  note: "active campaigns",
                },
                {
                  lbl: "Payout speed",
                  from: current.payoutSpeed,
                  to: next.payoutSpeed,
                  note: "",
                },
              ].map((row) => (
            ...
            {TIER_LADDER.map((t, i) => {
              ...
              return (
                <div key={t.tier} className={`an2-rung ${cls}`}>
                  <span
                    className="an2-rung__swatch"
                    style={{ background: t.swatch }}
                  />
                  <div>
                    <p className="an2-rung__name">{t.tier}</p>
                    <p className="an2-rung__material">
                      {t.material} · score {t.scoreFloor}+
                    </p>
                    <div className="an2-rung__meta">
                      <span>
                        <strong>{t.basePay}</strong> base
                      </span>
                      <span>
                        <strong>{t.commission}</strong> commission
                      </span>
                      {t.milestone && (
                        <span>
                          milestone <strong>+${t.milestone.bonus}</strong> @{" "}
                          {t.milestone.threshold} txns
                        </span>
                      )}
                      <span>
                        <strong>{t.concurrent}</strong> concurrent
                      </span>
                      <span>
                        <strong>{t.payoutSpeed}</strong> payouts
                      </span>
                    </div>
                  </div>
                  <div style={{ textAlign: "right" }}>
