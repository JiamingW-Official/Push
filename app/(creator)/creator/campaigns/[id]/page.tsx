...
/** Platform commission rate (%) earned on walk-in attribution */
...
/* ── Demo campaign data (8 campaigns) ───────────────────── */
...
/* ── Demo milestone state (per campaign) ─────────────────── */
// In a real app this comes from campaign_applications table
...
/* ── MilestoneActionArea sub-component ────────────────────── */
...
function MilestoneActionArea({ milestone }: { milestone: MilestoneStatus }) {
  ...
  > = {
    ...
    visited: {
      label: "You Visited — Submit Proof",
      cta: "Submit Proof",
    },
    proof_submitted: {
...
export default function CampaignDetailPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (isDemo) {
        ...
        // Check if demo campaign has applied milestone state
      ...
      try {
        ...
        // Check if already applied + get milestone
  ...
  async function handleApply() {
    ...
    if (isDemo) {
      // Optimistic demo state — no DB write
  ...
  return (
    <div className="campaign-page">
      ...
      <div className="cp-header">
        <div className="cp-header-inner">
          ...
          <div className="cp-meta">
            ...
            {/* TierBadge from shared component */}
            <TierBadge
      ...
      {/* ── Milestone Tracker (shown when applied) ──────────── */}
      {applied && milestone && <MilestoneTrack current={milestone} />}

      {/* ── Hero image (shown when not applied, or no milestone) */}
      {!applied && (
      ...
      <div className="campaign-body">
        ...
        <div className="campaign-main">
          ...
          {/* Requirements — enhanced cp-req cards */}
          <div>
              ...
              {noCommission ? (
                <>
                  ...
                  {campaignsToOperator > 0
                    ? `Complete ${campaignsToOperator} more campaign${
                        campaignsToOperator !== 1 ? "s" : ""
                      } to reach Operator and unlock 3% commission.`
      ...
      {/* Campaign checklist — shown when application is accepted/visited */}
      {applied && campaign && (
